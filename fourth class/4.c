...
void printShortestPath(Path *ppath,int paths[][2],int m); // 输出最短路径信息
...
int main()
{
    ...
    // 输出最短路径
    printShortestPath(ppath,paths,m);
...
// 输出最短路径信息
void printShortestPath(Path *ppath,int paths[][2],int m)
{
    for(int i=0;i<m;i++){
        for(int j=paths[i][0];;j=ppath->nextvex[j][paths[i][1]]){
            printf("%d",j);
            if(j!=paths[i][1] || i!=m-1) printf("\n");
            if(j==paths[i][1]) break;
        }
    }
}